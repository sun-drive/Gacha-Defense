document.addEventListener('DOMContentLoaded', () => {
    ...
    class Enemy {
        constructor(type) {
            ...
            this.x = startNode.x;
            this.y = startNode.y;
            this.width = type.isStickyHealer ? tileSize * 0.6 : tileSize * 0.8; // Smaller sticky healer
            ...
            this.hasStickyHealer = false; // For enemies that get a sticky healer attached
        ...
        updateAbilities() {
            ...
            if (this.isStickyHealer) {
                // Find a nearby non-boss, non-sticky-healer enemy that doesn't have a healer yet
                ...
                if (targetEnemy) {
                    if (Math.random() < 0.5) { // 50% chance to attach
            ...
            if (this.isBoss && this.abilityCooldown <= 0) {
                switch (this.type.name) {
                    case '헌터':
                        // Stun 2 nearest towers for 1 second
                        units.sort((a, b) => {
                        ...
                        for (let i = 0; i < 2 && i < units.length; i++) {
                            units[i].stunTimer = 60; // 1 second stun
                        }
                        showNotification('헌터가 타워 2개를 기절시킵니다!');
                        this.abilityCooldown = 180; // 3 second cooldown
                        break;
                    case '쉴더':
                        // Become invincible for 2 seconds and reduce tower damage
                        ...
                        this.abilityCooldown = 360; // 6 second cooldown
                        break;
                    case '더 킹':
                        // Summon enemies
                        ...
                        this.abilityCooldown = 300; // 5 second cooldown
                        break;
                }
            }
        }
    ...
    class FreezerTower extends BaseTower {
        attack() {
            ...
            enemies.forEach(enemy => {
                ...
                const distance = Math.sqrt(Math.pow(this.x - enemy.x, 2) + Math.pow(this.y - enemy.y, 2));
                if (distance <= this.stats.range) {
                    // Only apply freeze if enemy is NOT already slowed AND NOT resistant AND NOT immune
                    if (!enemy.slowed && !enemy.isSlowResistant && !enemy.type.isImmuneToFreeze) {
    ...
    const waves = [
        ...
        // Wave 36: Healer limit applied, new enemies added
        ...
        // Wave 44: Healer limit applied, Boss Warning
    ...
    function startNextWave() {
        ...
        // Boss Warning
        const nextWave = waves[wave]; // wave is 0-indexed for the array
        if (nextWave && nextWave.some(enemyName => enemyName.startsWith('boss_'))) {
            const bossName = enemyTypes[nextWave[0]].name;
            showNotification(`경고: 다음 웨이브에 ${bossName}이(가) 출현합니다!`);
        }
        ...
        if (isHardMode && wave > 19) {
            const extraEnemiesCount = (wave - 19) * 2; // Wave 20 -> 2 extra, 21 -> 4, etc.
        ...
        const spawnInterval = setInterval(() => {
            ...
            if (isHardMode) {
                if (wave >= 30) {
                    ...
                    healthMultiplier = 1.5; // On Hard Mode waves 20-29, health is 1.5x
                }
            }
    ...
    function handleProjectiles() {
        for (let i = projectiles.length - 1; i >= 0; i--) {
                ...
                if (Math.sqrt(dx * dx + dy * dy) < p.speed) {
                    ...
                    enemies.forEach(enemy => {
                        ...
                        if (distToEnemy <= p.blastRadius) {
                            let damageToApply = p.damage * globalDamageMultiplier;
                            if (enemy.isHeavyTank) damageToApply = Math.max(0, damageToApply - enemy.damageReduction);
            ...
            // --- Homing & Piercing Logic ---
            if (p.target && p.target.health > 0) {
                const dist = Math.sqrt(Math.pow(p.x - p.target.x, 2) + Math.pow(p.y - p.target.y, 2));
                if (dist < p.target.width / 2) {
                    hit = true;
                    hitTarget = p.target;
                }
            } else if (!p.target) {
                 // For projectiles that don't home on a single target (like area-of-effect or line attacks not yet implemented)
                 // Or if the original target is gone, could implement re-targeting here.
            }


            if (hit && hitTarget) {
                if (hitTarget.isInvincible) {
                    projectiles.splice(i, 1); // Projectile is consumed but does no damage
                    continue;
                }

                let damageToApply = p.damage * globalDamageMultiplier;
                if (hitTarget.isHeavyTank) damageToApply = Math.max(0, damageToApply - hitTarget.damageReduction);
                
                hitTarget.health -= damageToApply;
                playSound('hit');

                // --- Special Enemy Hit Logic ---
                if (hitTarget.isShockwave) {
                    hitTarget.hitsTaken++;
                    if (hitTarget.hitsTaken >= 4) {
                        hitTarget.hitsTaken = 0;
                        showNotification('충격파 발생!');
                        explosions.push(new Explosion(hitTarget.x, hitTarget.y, 40)); // Visual effect
                        units.forEach(unit => {
                            const distance = Math.sqrt(Math.pow(unit.x - hitTarget.x, 2) + Math.pow(unit.y - hitTarget.y, 2));
                            if (distance <= 40) {
                                unit.stunTimer = 120; // 2 second stun
                            }
                        });
                    }
                }

                // Handle projectile removal (pierce vs normal)
                if (p.pierce > 0) {
                    p.pierce--;
                    if (!p.hitEnemies) p.hitEnemies = [];
                    p.hitEnemies.push(hitTarget);
                    // Don't remove projectile yet
                } else {
                    projectiles.splice(i, 1);
                }
            }
            ...
            for (let j = enemies.length - 1; j >= 0; j--) {
                ...
                if (enemy.health <= 0) {
                    // Sticky Healer Revive Logic
                    if (enemy.hasStickyHealer) {
                        enemy.health = enemy.maxHealth / 2;
                        enemy.hasStickyHealer = false;
                        showNotification(`${enemy.type.name}이(가) 부활합니다!`);
                        continue; // Skip death processing for this frame
                    }
    ...
    function updateBossUI() {
        if (!bossInfoContainer) return; // Prevent error if element is not found
    ...
    function gameLoop() {
        ...
        // Find and set current boss
        if (!currentBoss) {
            currentBoss = enemies.find(e => e.type.isBoss) || null;
        }
        ...
        if (!waveSpawning && enemies.length === 0 && wave < waves.length) {
            ...
            if (wave > 0 && wave < 20 && !waveBonusGiven.includes(wave)) {
                const bonus = wave * 10;
                currency += bonus;
