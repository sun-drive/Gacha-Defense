document.addEventListener('DOMContentLoaded', () => {
    ...
    let sellUnitButton = null; // Declare globally
    let moveUnitButton = null; // Declare globally
    ...
    const towerData = {
        // Common
        'basic': { name: '기본 타워', rarity: 'common', color: '#A9A9A9', damage: 25, range: 110, fireRate: 40 },
        'tesla': { name: '테슬라 타워', rarity: 'common', color: '#00CED1', damage: 20, range: 80, fireRate: 90 }, // 1.5s
        'bomb': { name: '폭탄 타워', rarity: 'common', color: '#696969', damage: 18, range: 120, fireRate: 60, blastRadius: 20 }, // 1s
        // Rare
        'flame': { name: '화염 타워', rarity: 'rare', color: '#FF4500', damage: 10, range: 100, fireRate: 30 },
        'glue': { name: '접착 타워', rarity: 'rare', color: '#7CFC00', damage: 0, range: 80, fireRate: 60 },
        'ice': { name: '냉각 타워', rarity: 'rare', color: '#ADD8E6', range: 60, slowFactor: 0.5, slowDuration: 120, damage: 0, fireRate: 90 }, // 2 seconds
        // Epic
        'laser': { name: '레이저 발사기', rarity: 'epic', color: '#FF1493', damage: 4, range: 150, fireRate: 20 },
        'sword': { name: '검기 타워', rarity: 'epic', color: '#DB7093', damage: 35, range: 50, fireRate: 40 },
        'sniper': { name: '저격 타워', rarity: 'epic', color: '#8B4513', damage: 40, range: 9999, fireRate: 180 }, // Large range for farthest target
        // Legendary
        'gatling': { name: '개틀링', rarity: 'legendary', color: '#FFD700', damage: 15, range: 120, fireRate: 10 },
        'freezer': { name: '급속냉동기', rarity: 'legendary', color: '#00FFFF', damage: 0, range: 100, fireRate: 90 },
        'lightning': { name: '번개 생성기', rarity: 'legendary', color: '#9400D3', damage: 50, range: 130, fireRate: 70 },
    };
    ...
    class LightningTower extends BaseTower {
        attack() {
            ...
            if (this.target) {
                ...
                for (let i = 0; i < 2; i++) { // Chain to 2 more enemies
                    let nearestEnemy = null;
                    let minDistance = Infinity;

                    enemies.forEach(enemy => {
                        if (enemy !== chainTarget) { // Don't hit the same enemy twice in a chain
                            const distance = Math.sqrt(Math.pow(chainTarget.x - enemy.x, 2) + Math.pow(chainTarget.y - enemy.y, 2));
                            if (distance <= 80 && distance < minDistance) { // Chain range 80
                    ...
                    if (nearestEnemy) {
                        nearestEnemy.health -= this.stats.damage * 0.5; // Half damage for chained hits
                        chainTarget = nearestEnemy; // Next chain starts from this enemy
                    } else {
                        break; // No more enemies in chain range
                    }
                }
            }
        }
    ...
    function updateSelectedUnitInfo(unit) {
        ...
        if (isPlacedUnit) {
            ...
            // Get references and attach listeners here, ensuring elements are available
            sellUnitButton = document.getElementById('sell-unit-button'); // Assign to global
            moveUnitButton = document.getElementById('move-unit-button'); // Assign to global

            if (sellUnitButton && !sellUnitButton.hasAttribute('data-listener-attached')) {
                sellUnitButton.addEventListener('click', handleSellUnit);
                sellUnitButton.setAttribute('data-listener-attached', 'true');
            }
            if (moveUnitButton && !moveUnitButton.hasAttribute('data-listener-attached')) {
                moveUnitButton.addEventListener('click', handleMoveUnit);
                moveUnitButton.setAttribute('data-listener-attached', 'true');
            }
    ...
    function handleSellUnit() {
        ...
        if (unitIndex > -1) {
            ...
            currency += 100; // Add 100 currency
            flashElement(currencyEl, 'green');
            showNotification('유닛 판매! +100 재화');
        }
