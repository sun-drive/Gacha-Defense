document.addEventListener('DOMContentLoaded', () => {
    ...
    const maps = {
        ...
        map2: [ // ㄹ shape
            ...
            { x: mapWidth - tileSize * 2 + 0.5, y: tileSize * 2 + tileSize / 2 },
            { x: mapWidth - tileSize * 2 + 0.5, y: tileSize * 6 + tileSize / 2 },
            { x: tileSize * 2 + 0.5, y: tileSize * 6 + tileSize / 2 },
            { x: tileSize * 2 + 0.5, y: tileSize * 10 + tileSize / 2 },
            { x: mapWidth, y: tileSize * 10 + tileSize / 2 },
        ],
        map3: [ // Loop shape
            { x: 0, y: tileSize * 3 + tileSize / 2 },
            { x: mapWidth - tileSize * 4 + 0.5, y: tileSize * 3 + tileSize / 2 },
            { x: mapWidth - tileSize * 4 + 0.5, y: mapHeight - tileSize * 4 + tileSize / 2 },
            { x: tileSize * 4 + 0.5, y: mapHeight - tileSize * 4 + tileSize / 2 },
    ...
    const towerData = {
        ...
        'tesla': { name: '테슬라 타워', rarity: 'common', color: '#00CED1', damage: 20, range: 80, fireRate: 90, description: '범위 내 모든 적에게 전기 피해를 입힙니다. 5레벨에 번개 생성기로 각성할 수 있습니다.' },
        ...
        'magnifying_glass': { name: '돋보기 타워', rarity: 'epic', color: '#FBC02D', damage: 10, range: 110, fireRate: 180, description: '유령 적만 공격합니다. 이 타워의 상하좌우 1칸에 위치한 타워의 사거리를 40 증가시킵니다.' },
        
        // 각성 및 신규 타워
        'lava_tower': { name: '용암 타워', rarity: 'legendary', color: '#FF8C00', damage: 15, range: 0, fireRate: 420, description: '기지에서 7초마다 용암 유닛을 출격시켜 경로의 적에게 15의 피해와 3.5초간 15의 지속 피해를 줍니다.' },
        ...
        'sphinx': { name: '스핑크스', rarity: 'legendary', color: '#DAA520', damage: 5, range: 100, fireRate: 60, description: '1초 간격으로 6개의 발사체를 발사하여 2초간 지속 피해를 줍니다. 9초마다 5초간 랜덤한 색상의 버프를 받습니다.' },
    ...
    const upgradeCosts = {
        common: [100, 150, 220, 350, 500], // 5레벨 비용 추가
        rare:   [150, 250, 400, 700, 1100],
        epic:   [250, 450, 750, 1300, 2000],
        legendary: [500, 900, 1600, 3000, 5000]
    };
            ...
            enemies.forEach(enemy => {
                ...
                if (distance <= this.stats.range) {
                    if (!enemy.slowed && !enemy.isSlowResistant && !enemy.type.isImmuneToFreeze) {
                        enemy.slowed = true;
                        enemy.slowTimer = 180;
                        enemy.appliedSlowFactor = 0;
                        enemy.slowResistanceTimer = 180;
    ...
    class LightningTower extends BaseTower {
        attack() {
            ...
            if (enemiesInRange.length > 0) {
                ...
                targetEnemy.magnetismTimer = 180;
                showNotification(`${targetEnemy.type.name}에게 자성 부여!`);
            }
        }
    }
    ...
    class LavaRock {
        ...
        move() {
            if (this.pathIndex >= currentPath.length - 1) return true; // Mark for deletion
    ...
    class LavaTower extends BaseTower {
        attack() {
            ...
            this.fireCooldown = this.stats.fireRate;
            const startNode = currentPath[0];
            lavaRocks.push(new LavaRock(startNode.x, startNode.y));
        }
    }
    ...
    class SphinxTower extends BaseTower {
        constructor(x, y, towerId) {
            super(x, y, towerId);
            this.specialAbilityTimer = 540; // 9 seconds
        ...
        update() {
            ...
            if (this.specialAbilityTimer > 0) {
                ...
                this.specialAbilityTimer = 540; // Reset timer
    ...
    class WaterBombTower extends BaseTower {
        attack() {
            this.fireCooldown = this.stats.fireRate;
            explosions.push(new Explosion(this.x, this.y, this.stats.blastRadius));
            enemies.forEach(enemy => {
                const dx = this.x - enemy.x;
                const dy = this.y - enemy.y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                if (distance < this.stats.blastRadius) {
                    enemy.health -= this.stats.damage;
                    if (enemy.slowed && enemy.appliedSlowFactor <= 0.5) return; // Max 4 stacks
                    enemy.slowed = true;
                    enemy.slowTimer = 180; // Keep refreshing slow
    ...
    function getPointAheadOnPath(enemy, distanceToJump) {
        let remainingDistance = distanceToJump;
        let currentPos = { x: enemy.x, y: enemy.y };
        let pathIndex = enemy.pathIndex;

        while (remainingDistance > 0 && pathIndex < currentPath.length - 1) {
            ...
            if (remainingDistance <= segmentLength) {
                ...
                currentPos.x += segmentDx * ratio;
                currentPos.y += segmentDy * ratio;
                remainingDistance = 0;
            } else {
        ...
        return { x: currentPos.x, y: currentPos.y, pathIndex: pathIndex };
    }
    ...
    function isPointOnPath(x, y) {
        for (let i = 0; i < currentPath.length - 1; i++) {
            ...
            if (dist < tileSize / 2) { // Check if point is within half a tile of the path line
                return true;
            }
        }
        return false;
    ...
    function handleAwaken() {
        ...
        if (!awakenedTowerId || selectedPlacedUnit.level !== 5 || !isHardMode) {
            showNotification("각성 조건을 만족하지 못했습니다.");
            return;
        }
        ...
        if (unitIndex !== -1) {
            ...
            newUnit.level = 5; // 각성된 타워는 5레벨부터 시작
    ...
    function handleSellUnit() {
        if (selectedPlacedUnit) {
            const sellValue = Math.floor(upgradeCosts[selectedPlacedUnit.stats.rarity].slice(0, selectedPlacedUnit.level - 1).reduce((a, b) => a + b, 0) * 0.7);
            currency += sellValue;
            updateUI();
    ...
    function handleUpgrade() {
        ...
        if (currentLevel >= 5) { // 5레벨 이상 업그레이드 불가 (각성으로 대체)
            showNotification("최대 레벨입니다. 각성을 시도하세요.");
            return;
        }
